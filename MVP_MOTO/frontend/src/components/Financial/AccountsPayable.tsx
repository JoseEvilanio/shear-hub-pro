...
import { AccountPayable, PaymentData } from '@/services/financialService';
...
interface AccountsPayableProps {
  ...
  loading?: boolean;
  onPayAccount?: (id: string, paymentData: PaymentData) => Promise<boolean>;
  onEditAccount?: (account: AccountPayable) => void;
...
export const AccountsPayable: React.FC<AccountsPayableProps> = ({
  ...
  const handlePayAccount = (account: AccountPayable) => {
    ...
    setPaymentData({
      ...
      paymentMethod: 'cash',
      notes: lateFee ? `Pagamento com ${daysLate} dias de atraso. Multa: ${formatCurrency(lateFee.fine)}, Juros: ${formatCurrency(lateFee.interest)}` : '',
    });
  ...
  const handleSubmitPayment = async () => {
    if (!selectedAccount || !onPayAccount) return;

    try {
      setPaymentLoading(true);
      await onPayAccount(selectedAccount.id, paymentData);
                  ...
                  return (
                    <tr key={account.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {account.supplier?.cnpj && (
                          <div className="text-sm text-gray-500">
                            CNPJ: {account.supplier.cnpj}
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          {account.status === 'pending' && onPayAccount && (
                            <button
        ...
        {selectedAccount && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-900 mb-3">Fornecedor</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-500">Nome:</span>
                    <span className="font-medium">{selectedAccount.supplier?.name || 'N/A'}</span>
                  </div>
                  {selectedAccount.supplier?.cnpj && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">CNPJ:</span>
                      <span className="font-medium">{selectedAccount.supplier.cnpj}</span>
                    </div>
                  )}
                </div>
              </div>
